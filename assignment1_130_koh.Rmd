...
## Introduction
'The Development Disorders Genotype-Phenotype Database(DDG2P)'란 'developmental disorders'와 연관된 것으로 보고된 gene들의 curated list이다. 이는 DDD(Deciphering Development Disorders) study의 한 부분으로서 likely causal variants를 분석하기 위해 편집된 data이다.
...
결과적으로 `mutation.consequence`에는 총 8가지 mutation consequence가 item으로 존재함을 알 수 있다.
...
이렇게 총 26가지 organ이 item으로 존재함을 알 수 있다.

세번째로 `pLI`라는 이름의 column에 들어있는 `pLI score`에 대해 알아보겠다.
`pLI score`이란 '특정 유전자에 loss of function이 발생했을 때 정상적으로 작동하지 않을 확률'이다. 따라서 `pLI score`이 높은 유전자는 약간의 돌연변이가 발생하더라도 유전자에 큰 영향을 미칠 확률이 높다.
또한 `pLI score`에서 유의할 점은, 이 값은 gene specific하며 다른 mutation이 아닌 'loss of function'과만 관련이 있는 값이라는 사실이다.
...
그리고 `Studying the data`에서 알아본대로, `mutation.sequence`에서 'activating' = 'gain of function'이므로 'gain of function'을 모두 'activating'으로 바꿔 통일해준다. 또한 'NA'들은 빈도수를 비교함에 있어서는 무의미하므로 모두 삭제해준다.
...
앞서 공부한 내용에 따르면, `pLI score`란 '특정 유전자에 loss of function이 발생했을 때 정상적으로 작동하지 않을 확률'이므로 gene에 specific한 값임을 알 수 있다. 그러나 필자는 이 사실을 한 번 더 확인하기 위해 같은 gene.symbol에 대해 다른 pLI값을 가지는 경우가 있는지 확인하겠다.
...
```{r}
pLI_organ %>% ggplot(aes(x = organ.specificity.list, y = avg_pLI)) + geom_bar(stat = 'identity') + coord_flip() + theme(axis.text.y = element_text(size = 6)) + xlab("") + ggtitle("Average pLI score according to the organ") 
```
