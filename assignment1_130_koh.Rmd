...
```{r echo=TRUE}
analysis_2 <- pLI_organ %>% ggplot(aes(x = organ.specificity.list, y = avg_pLI)) + geom_bar(stat = 'identity') + coord_flip() + theme(axis.text.y = element_text(size = 6)) + xlab("") + ggtitle("Average pLI score according to the organ") + theme(plot.title = element_text(size = 9, face = "bold"))
