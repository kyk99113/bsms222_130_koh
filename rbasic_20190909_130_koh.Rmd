...
To summarize, let's look at the three functions we have introduced:
...
ind <- order(pop)
murders$state[ind]
rank(murders$state[ind])
ranks <- rank(murders$state[ind])
...
```{r}
cels <- 5/9*(temp-32)
city_temps <- data.frame(name=city, temperature=cels)
```
...
```{r}
population_in_log10 <- log(murders$population, base=10)
total_gun_murders <- murders$total
