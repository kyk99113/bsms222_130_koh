...
```{r}
#1. 유전자(gene)들만 선택을 합니다. 여기에서 gene_id, gene_name, hgnc_id, gene_type, chromosome, start, end, strand를 갖는 data frame을 만들고, txt 파일로 저장합니다. 이때 hgnc 정보가 부재한 유전자들은 0으로 표시합니다. 
...
d1$hgnc_id <- ifelse(grepl('HGNC', d1$info), 1, 0)
...
d5 <- select(d1, chrom, start, end, strand, gene_id, gene_name, hgnc_id, gene_type)

write.table(d5, 'table.gene_genecode_plusinfo.20190923.txt', sep='\t', quote=F, row.names = F, col.names = T)
```

```{r}
#2. gene biotype에 따라 hgnc 정보 존재 여부를 계수합니다.

d5 %>% group_by(gene_type) %>% summarize(n = sum(ifelse(hgnc_id == 1, 1, 0)))
```

```{r}
#3. hgnc 여부에 따라 level을 나눕니다. level은 transcript_support_level이 아니라, 그냥 level입니다.

d0 <- separate(d1, info, sep = ';', into = c('one','two','three','four'))

d_level <- d0 %>% select('four', 'hgnc_id') %>% group_by(hgnc_id, four) %>% tally() 
