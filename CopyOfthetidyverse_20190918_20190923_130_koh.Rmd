...
Each row represent a state with each if the five columns providing a different variable related to these states: name, abbreviation, region, population, and total murders.
...
B. `ChickWeight` is tidy: each observation (a weight) is represented by one row. The chick from which this measurement came is one the variables.
...
```{r}
ref_avg <- NHANES %>% filter(AgeDecade == " 20-29", Gender == "female") %>% summarize(mean(BPSysAve, na.rm = TRUE)) %>% pull('mean(BPSysAve, na.rm = TRUE)')
ref_avg
```
...
```{r}
NHANES %>% filter(AgeDecade == " 20-29", Gender == "female") %>% summarize(min(BPSysAve, na.rm = TRUE)) %>% pull('min(BPSysAve, na.rm = TRUE)')

NHANES %>% filter(AgeDecade == " 20-29", Gender == "female") %>% summarize(max(BPSysAve, na.rm = TRUE)) %>% pull('max(BPSysAve, na.rm = TRUE)')
```
...
```{r}
NHANES %>% filter(Gender == "female") %>% group_by(AgeDecade) %>% summarize(mean(BPSysAve, na.rm = TRUE), sd(BPSysAve, na.rm = TRUE)) 
```

5. Repeat exercise 4 for males.

```{r}
NHANES %>% filter(Gender == "male") %>% group_by(AgeDecade) %>% summarize(mean(BPSysAve, na.rm = TRUE), sd(BPSysAve, na.rm = TRUE)) 
```
...
```{r}
NHANES %>% group_by(AgeDecade, Gender) %>% summarize(mean(BPSysAve, na.rm = TRUE), sd(BPSysAve, na.rm = TRUE)) 
```
...
```{r}
NHANES %>% filter(Gender == "male", AgeDecade == " 40-49") %>% group_by(Race1) %>% summarize(mean(BPSysAve, na.rm = TRUE))
```
...
```{r}
murders %>% group_by(region) %>% do(as_tibble(.))
```
...
```{r}
v <- data.frame(n = 1:100, s_n <- map_df(n, compute_s_n), s_n_2 <- map_df(n, compute_s_n))
v
```
