...
Suppose a colleague is charged with collecting demography(인구통계학) data for a group of males. The data report height in feet and are stored in the object:
...
Keep in mind that this is not such an extreme event: if we have 100 data points that are normally distributed, we expect to see about 7 outside of this range. But these would not be outliers since we expect to see them under the typical variation.
...
```{r}
(quantile(x_with_error, 0.50)-quantile(x, 0.50))
```
