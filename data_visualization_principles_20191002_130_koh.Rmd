...
We start by describing some principles for encoding data. There are several approaches at our disposal(배열) including position, aligned lengths, angles, area, brightness, and color hue.
...
Notice how much easier it is to see the differences in the barplot. In fact, we can now determine the actual percentages by following a horizontal line to the y-axis.
...
(Source: Fox News, via [Media Matters](https://www.mediamatters.org/fox-news/fox-news-newest-dishonest-chart-immigration-enforcement). )

From the plot above, it appears that apprehensions(불안) have almost tripled when, in fact, they have only increased by about 16%. Starting the graph at 0 illustrates this clearly:

![](https://rafalab.github.io/dsbook/book_files/figure-html/barplot-from-zero-1-1.png)

Here is another example, described in detail in this [Flowing Data blog post](https://flowingdata.com/2012/08/06/fox-news-continues-charting-excellence/)
...
Pseudo-3D is sometimes used completely gratuitously(쓸데없이): plots are made to look 3D even when the 3rd dimension does not represent a quantity. This only adds confusion and makes it harder to relay your message. Here are two examples:
...
|California|1980|0.3743467|0.051546|0.0515466|

We are reporting precision up to 0.00001 cases per 10,000, a very small value in the context of the changes that are occurring across the dates. In this case, two significant figures are more than enough and clearly makes the point that rates are decreasing:
...
4. To make the plot on the right, we have to reorder the levels of the states’ variables.
...
```{r}
murders %>% group_by(region) %>%  summarize(median_rate = median(rate)) %>% mutate(region = reorder(region, median_rate)) %>%  ggplot(aes(region, median_rate)) + geom_boxplot()
```
...
sp <- us_contagious_diseases %>%  
  filter(weeks_reporting >= 10 & disease=="Smallpox") %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting, state = reorder(state, rate))

sp %>% ggplot(aes(state, rate)) +
  geom_bar(stat="identity") +
  coord_flip()
```
...
```{r}
sp %>% ggplot(aes(year, rate)) + geom_point() 
```
...
```{r}
us_contagious_diseases %>% mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% filter(state == "California" & weeks_reporting >= 10) %>% ggplot(aes(rate, year)) + geom_point(aes(col = disease)) + scale_x_log10()
```
...
ee <- us_contagious_diseases %>%
  filter(!state%in%c("Hawaii","Alaska") & disease == the_disease & weeks_reporting  >= 10) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate)) 
ee %>% filter(!is.na(rate)) %>%
  ggplot(aes(year, rate)) +
  geom_line() + 
  ylab("Cases per 10,000") 
```
...
```{r}
ee %>% ggplot(aes(year, state, fill = rate)) +
  geom_tile(color = "grey50") +
  scale_x_continuous(expand=c(0,0)) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Reds"), trans = "sqrt") +
  theme_minimal() +  theme(panel.grid = element_blank()) +
...
```{r}
zz <- us_contagious_diseases %>%
  filter(!state%in%c("Hawaii","Alaska") & state == "California" & weeks_reporting  >= 10) %>%
  mutate(rate = count / population * 10000 * 52 / weeks_reporting) %>% 
  mutate(state = reorder(state, rate)) 
zz %>% ggplot(aes(year, disease, fill = rate)) +
  geom_tile(aes(color = disease)) +
  scale_x_continuous(expand=c(0,0)) +
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(9, "Reds"), trans = "sqrt") +
  theme_minimal() +  theme(panel.grid = element_blank()) +
  ggtitle("California") + 
...
```{r}
urate <- us_contagious_diseases %>% group_by(year) %>%
  summarize(us_rate = sum(count, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000)

zz %>% 
  filter(!is.na(rate)) %>%
    ggplot() +
  geom_line(aes(year, rate, group = disease),  color = "grey50", 
            show.legend = FALSE, alpha = 0.2, size = 1) +
  geom_line(mapping = aes(year, us_rate),  data = urate, size = 1, color = "black") +
  scale_y_continuous(trans = "sqrt", breaks = c(5, 25, 125, 300)) + 
  ggtitle("Cases per 10,000 by state") + 
  xlab("") + 
  ylab("") +
