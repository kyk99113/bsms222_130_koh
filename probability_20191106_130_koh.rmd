...
6. To see the math that explains why the answers to questions 3, 4, and 5 are the same, suppose we have a random variable with average _m_ and standard error _s_. Suppose we ask the probability of X being smaller or equal to a. Remember that, by definition, a is (a−m)/s standard deviations s away from the average m. The probability is:
