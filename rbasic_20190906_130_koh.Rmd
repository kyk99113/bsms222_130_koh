...
If using the arguments' namesm then we can include them in whatever order we want:
...
Among these, the mose commonly used are the arithmetic and relational operators. For example:
...
In a numeric vectorm every entry must be a number.
...
Another important type of vectors are _logical_ vectors. These must be either **True** of **False**.
...
Here the **==** is a relational operator asking if 3 is equal to 2. In R, if you just use one **=**, tou actually assign a variable, but if you use two **==** you test for equality.
...
In futere sections, you will see how useful relational operators can be.
...
**Warning**: Factors can be a source of confusion since sometimes they bahave like characters and sometimes they do not. As a result, confusing factors and characters are a common source of bugs.
