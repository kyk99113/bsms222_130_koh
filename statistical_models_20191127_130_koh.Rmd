...
To which Nate Silver reponded via Twitter:
...
$$Y_ij=d+b_i+\varepsilon_{i,j}$$

with $i=1,2$ indexing the two pollsters, $b_i$ the bias for pollster $i$ and $\varepsilon_{ij}$ poll to poll chance variability. We assume the $\varepsilon$ are independent from each other, have expected value 0 and standard deviation $\sigma_1$ regardless of $j$.
...
10. Suppose we define $\bar{Y}_1$ as the average of poll results from the fist poll, $Y_{1,1}, ..., Y_{1,N_1}$ with $N_1$ the number of polls conducted by the first pollster:
...
**Answer**: $$ \mbox{E}[\bar{Y}_{2} - \bar{Y}_1] = \mbox{E}[\bar{Y}_{2}] - \mbox{E}[\bar{Y}_1] = (d + b_2) - (d + b_1) = b_2 - b1$$

15. Using what we learned by answering the questions above, what is the standard error of  $\bar{Y}_2-\bar{Y}_1$?

**Answer**: $$ \mbox{SE}[\bar{Y}_{2} - \bar{Y}_1] = \sqrt{\mbox{SE}[\bar{Y}_{2}]^2 + \mbox{SE}[\bar{Y}_1]^2} = \sqrt{\sigma_2^2/N_2 + \sigma_1^2/N_1}$$
...
b. Because the $Y_{1,j}$ are approximately normal, so are the averages.
